...
from schemas import MergedStyle, OllamaClient, OUTLINE_LABELS
...
def _run_job(job_id: str) -> None:
    ...
    _update_job(
        ...
        status="running",
        progress=8,
        message="正在检查本地 Ollama 模型...",
    )

    llm = OllamaClient()
    health = llm.health(timeout=5)
    if not health.get("ok"):
        msg = health.get("msg", "Ollama 检查失败")
        _update_job(
            job_id,
            status="failed",
            progress=100,
    ...
    _update_job(
        job_id,
        progress=9,
        message=f"本地模型已就绪: {health.get('model')}",
    )
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            ollama = OllamaClient().health(timeout=2)
            self._send_json(
                HTTPStatus.OK,
                {
                    "code": 200,
    ...
    def _delete_job(self, job_id: str) -> None:
        ...
        for base_dir in (UPLOAD_DIR, OUTPUT_DIR):
            target = (base_dir / job_id).resolve()
            try:
                target.relative_to(base_dir.resolve())
            except ValueError:
                self._send_api_error(HTTPStatus.FORBIDDEN, "禁止删除该路径")
                return
            if target.exists():
        ...
        self._send_json(
            HTTPStatus.OK,
            {"code": 200, "msg": "历史文件已删除", "jobId": job_id},
        )
