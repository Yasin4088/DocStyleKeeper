...
window.addEventListener('DOMContentLoaded', function() {
    ...
    async function checkBackendStatus() {
        ...
        try {
            ...
            ollamaReady = Boolean(data.ollama && data.ollama.ok);
            if (backendReady && !ollamaReady) {
                setBackendStatus('offline', data.ollama?.msg || '本地后端已启动，但 Ollama 模型不可用');
                return false;
            }
        } catch (e) {
            backendReady = false;
        ...
        if (backendReady && ollamaReady) {
            setBackendStatus('online', '本地后端与 Ollama 模型已连接，可以上传并处理 Word 文件');
        } else {
            setBackendStatus('offline', '本地后端未启动：请先运行“启动网页.bat”');
        }
        return backendReady && ollamaReady;
    }
    ...
    function resetProgress() {
        ...
        progressMessage.textContent = '提交任务后会显示本地模型处理进度。';
    ...
    function getStyleConfig() {
        return {
            content: {
                font_cn: document.querySelector('.font_cn_content').value,
                font_en: document.querySelector('.font_en_content').value,
                size: document.querySelector('.size_content').value,
                line: document.querySelector('.line_content').value,
                first_line_indent: document.querySelector('.indent_firstline')?.value || '2',
                space_before: document.querySelector('.space_before')?.value || '0',
                space_after: document.querySelector('.space_after')?.value || '0',
                align: document.querySelector('.align_content')?.value || 'both'
            },
            h1: {
                font_cn: document.querySelector('.font_cn_h1').value,
                font_en: document.querySelector('.font_en_h1').value,
                size: document.querySelector('.size_h1').value,
                bold: document.querySelector('.bold_h1').value,
                first_line_indent: document.querySelector('.indent_h1')?.value || '0',
                space_before: document.querySelector('.space_before_h1')?.value || '0',
                space_after: document.querySelector('.space_after_h1')?.value || '0',
                align: document.querySelector('.align_h1')?.value || 'left'
            },
            h2: {
                font_cn: document.querySelector('.font_cn_h2').value,
                font_en: document.querySelector('.font_en_h2').value,
                size: document.querySelector('.size_h2').value,
                bold: document.querySelector('.bold_h2').value,
                first_line_indent: document.querySelector('.indent_h2')?.value || '0',
                space_before: document.querySelector('.space_before_h2')?.value || '0',
                space_after: document.querySelector('.space_after_h2')?.value || '0',
                align: document.querySelector('.align_h2')?.value || 'left'
            },
            h3: {
                font_cn: document.querySelector('.font_cn_h3').value,
                font_en: document.querySelector('.font_en_h3').value,
                size: document.querySelector('.size_h3').value,
                bold: document.querySelector('.bold_h3').value,
                first_line_indent: document.querySelector('.indent_h3')?.value || '0',
                space_before: document.querySelector('.space_before_h3')?.value || '0',
                space_after: document.querySelector('.space_after_h3')?.value || '0',
                align: document.querySelector('.align_h3')?.value || 'left'
            }
        };
    }
    ...
    function saveHistory(fileName, url, jobId = '') {
        const item = {
            name: fileName,
            time: new Date().toLocaleString(),
            url: url,
            jobId: jobId || getJobIdFromUrl(url)
        };
    ...
    async function deleteHistoryItem(index) {
        ...
        if (!item) return;
        const confirmed = confirm(`确定删除历史文件“${item.name}”吗？`);
        if (!confirmed) return;
        ...
        const jobId = item.jobId || getJobIdFromUrl(item.url);
        if (!jobId) return;
    ...
    startProcessBtn.onclick = async () => {
        ...
        const online = (backendReady && ollamaReady) || await checkBackendStatus();
        if (!online) {
            alert('本地后端或 Ollama 模型不可用，请确认服务已启动且模型已下载');
            return;
        }
        ...
        updateProgress({
            status: 'queued',
            progress: 5,
            message: '正在上传文件并提交本地 AI 处理任务...',
            logs: ['正在上传文件并提交本地 AI 处理任务...']
        });
        ...
        try {
            ...
            if (data.code === 202 && data.jobId) {
                ...
                saveHistory(
                    job.fileName || ('已排版_' + selectedFile.name),
                    job.downloadUrl,
                    job.jobId
                );
            } else {
